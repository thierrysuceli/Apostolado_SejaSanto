...
const Novenas = () => {
  ...
  const fetchData = async () => {
    try {
      const [novenasRes, columnsRes] = await Promise.all([
        get('/api/content?type=novenas'),
        get('/api/public-data?type=editorial-columns')
      ]);
      
      setNovenas(novenasRes.novenas || []);
  ...
  const filteredNovenas = selectedColumn
    ? novenas.filter(n => n.editorial_column?.id === selectedColumn)
    : novenas;

  const featuredNovena = filteredNovenas.find(n => n.is_featured);
  const regularNovenas = filteredNovenas.filter(n => !n.is_featured);
  ...
  return (
    <div className="min-h-screen bg-beige-50 dark:bg-gray-950 py-8 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-secondary-600 dark:text-gray-400 text-lg">
                Orações e devoções para fortalecer sua fé
              </p>
            </div>
            {isAdmin && (
              <button
                onClick={() => navigate('/admin/novenas/create')}
        ...
        {featuredNovena && (
          <div className="mb-12">
            <Link to={`/novenas/${featuredNovena.slug}`}>
              <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl overflow-hidden border border-beige-200 dark:border-gray-800 hover:shadow-3xl transition-all duration-300 transform hover:-translate-y-1">
                <div className="grid md:grid-cols-2 gap-0">
                  {featuredNovena.cover_image_url && (
                    <div className="h-64 md:h-full">
                      <img
                        src={featuredNovena.cover_image_url}
                        alt={featuredNovena.title}
                  ...
                  <div className="p-8 md:p-10 flex flex-col justify-center">
                    ...
                    <h2 className="text-3xl md:text-4xl font-bold text-secondary-700 dark:text-gray-200 mb-4 leading-tight">
                      {featuredNovena.title}
                    </h2>
                    {featuredNovena.excerpt && (
                    ...
                    <div className="flex items-center text-sm text-secondary-500 dark:text-gray-500">
                      ...
                      {new Date(featuredNovena.published_at).toLocaleDateString('pt-BR')}
                    </div>
                  </div>
                </div>
              </div>
        ...
        {/* Novenas Grid - Large Magazine Style */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {regularNovenas.map(novena => (
            <Link key={novena.id} to={`/novenas/${novena.slug}`}>
                    ...
                    <div className="absolute top-4 left-4 flex flex-wrap gap-2">
                      {novena.editorial_column && (
                        <span 
                          ...
                          style={{ backgroundColor: `${novena.editorial_column.color}dd` }}
                        >
                    ...
                    {novena.author && (
                      <div className="flex items-center">
                        {novena.author.avatar_url ? (
                          <img src={novena.author.avatar_url} alt={novena.author.name} className="w-8 h-8 rounded-full mr-2" />
                        ) : (
                        ...
                        <span className="text-sm font-semibold text-gray-900 dark:text-white">{novena.author.name}</span>
                      </div>
                    )}
        ...
        {filteredNovenas.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-secondary-600 dark:text-gray-400 text-lg">
              Nenhuma novena encontrada nesta coluna.
            </p>
          </div>
        )}
      </div>
