// =====================================================
// COMPONENTE: Gerenciador de Perguntas do Formulário
// =====================================================

import { useState, useEffect } from 'react';
import { useApi } from '../contexts/ApiContext';

export default function FormQuestionsManager({ registrationId }) {
  const api = useApi();
  const [questions, setQuestions] = useState([]);
  const [loading, setLoading] = useState(false);
  const [editingQuestion, setEditingQuestion] = useState(null);
  
  // Form state para nova pergunta
  ...
  const loadQuestions = async () => {
    try {
      setLoading(true);
      const data = await api.registrations.getQuestions(registrationId);
  ...
  const handleCreateQuestion = async () => {
    ...
    try {
      await api.registrations.createQuestion(registrationId, {
        ...newQuestion,
  ...
  const handleDeleteQuestion = async (questionId) => {
    ...
    try {
      await api.registrations.deleteQuestion(questionId);
      await loadQuestions();
    } catch (error) {
