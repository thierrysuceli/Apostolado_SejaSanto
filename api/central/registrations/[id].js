// =====================================================
// API - /api/central/registrations/[id]
// Endpoint dinâmico para ações em inscrições
// =====================================================
  ...
  try {
    ...
    if (req.method === 'POST' && action === 'subscribe') {
      ...
      if (regError || !registration) {
        console.error('[REGISTRATION SUBSCRIBE] Not found:', regError);
        return res.status(404).json({ error: 'Inscrição não encontrada' });
      }
      
      // Verificar se está ativa
      if (!registration.is_active) {
        console.log('[REGISTRATION SUBSCRIBE] Not active:', registration.id);
      ...
      console.log('[REGISTRATION SUBSCRIBE] Time check:', { 
        now: now.toISOString(), 
      ...
      if (insertError) {
        console.error('[REGISTRATION SUBSCRIBE] Insert error:', insertError);
        throw insertError;
      }
      
      // Se aprovação automática, dar a role
      if (registration.approval_type === 'automatic' && registration.role_to_grant) {
        ...
        if (!hasRole) {
          ...
          console.log('[REGISTRATION SUBSCRIBE] Role granted:', { userId: req.user.id, roleId: registration.role_to_grant });
        }
      }
      
      console.log('[REGISTRATION SUBSCRIBE] Success:', { registrationId, userId: req.user.id, status });
    ...
    if (req.method === 'DELETE' && action === 'delete') {
      ...
      console.log('[REGISTRATION DELETE] Success:', registrationId);
      return res.status(200).json({ 
    ...
    if (req.method === 'PUT' && action === 'edit') {
      ...
      console.log('[REGISTRATION UPDATE] Success:', registrationId);
      return res.status(200).json({ 
